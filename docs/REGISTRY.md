...
That matters more here than anywhere else in the repo, because the lactation id is derived from a calving event id, and a collision would give two lactations one id — the exact failure the id-stability decision exists to prevent, reintroduced through the id format instead of the numbering scheme.

> **Corrected.** This paragraph said step 4's yield rows would carry the lactation id as a **foreign key**. They do not. Correcting a calving supersedes it, so the derived lactation id changes and the rebuild deletes the old row; and with no correction at all, recording a calving re-cuts the *previous* lactation's `ended_on`, so yield already written into the overlap silently belongs to a different lactation while its key still names the old one — which no invariant can see, because both rows exist. Yield is keyed on `(animal_id, occurred_on, session)` and derives its lactation by date range. Both failures are demonstrated by execution in [REGISTRY_MILKING.md](REGISTRY_MILKING.md) §2. The general rule: `registry_lactations` is a projection the rebuild drops and recreates, so **a record must never carry a foreign key into it.**
...
```bash
npm run registry:harness -w server -- --port=6400   # fixture herd, in memory
npm start -w web-angular                            # the app on :6420, proxying /api
```

**`--port=6400` is not optional, and its absence was a live hazard.** The harness defaults to **6410**; `web-angular/proxy.conf.json` targets **6400**. So the harness on its default port is a process the app cannot see, and the app instead talks to whatever holds 6400 — which, if `npm run dev -w server` is up, is the real `dairy.db`. Measured: `GET :6410/api/registry/animals` returned the 13 fixture animals while `GET :6400/api/registry/animals` returned `{"animals":[]}` from the live database, at the same moment.
