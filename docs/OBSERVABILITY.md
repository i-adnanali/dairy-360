...
- ~~No decision yet on eval/regression tooling (golden-dataset assertions on tool-call sequences) — that's Cycle 2~~ **Resequenced.** Cycle 2 became the multi-agent system (vendor/sales agent + dispatcher + reconciliation; see [MULTI_AGENT.md](MULTI_AGENT.md), tagged `v0.5.0`). Eval/regression tooling is now Cycle 3, deliberately following the multi-agent work so it could be scoped against a shape that had stabilized rather than one still in flux. **Done** — see [REGRESSION.md](REGRESSION.md) (tagged `v0.6.0`): a live-model suite of 12 scenarios (dispatcher routing, reconciliation accuracy, write gates, plus reused Cycle 1 cases). As predicted, it asserts against one transport's AG-UI event sequences, since there remains a single path through the app.
