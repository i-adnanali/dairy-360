...
- **No agent tool reads them.** The registry is a service layer plus its own UI; the diagram below is the schema the agent's tools see, which is why the `registry_*` tables are not in it.
