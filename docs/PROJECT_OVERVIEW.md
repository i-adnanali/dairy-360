...
- **The agent reads them, and only reads them.** Cycle 9 added three read tools over the animal registry and the sales cycle added four more over the destinations and the ledger; the labour tables have **none at all**, and for a different reason — the chat panel has no notion of who is using it, and salaries are the first data in this repo where that matters ([REGISTRY_PAYROLL.md §14](REGISTRY_PAYROLL.md#14-build-order)). Every registry write still goes through the entry screens. The diagram below is the *demo* schema, which is why the `registry_*` tables are not in it — their schema belongs to REGISTRY.md and the two feature documents.
- **There are three axes, not one sequence.** The animal record is a sequence of steps; the sales tables (`registry_destinations`, `registry_destination_prices`, `registry_dispatches`, `registry_payments`) hang off a **counterparty**; the labour tables (`registry_people`, `registry_engagements`, `registry_pay_terms`, `registry_pay_benefits`, `registry_wage_periods`, `registry_wage_payments`) hang off a **person the farm employs**. Neither of the last two carries a step number, because neither is a later step along the first.
