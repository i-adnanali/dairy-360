...
Cycle 8 (animal registry, see [REGISTRY.md](REGISTRY.md)) adds `registry_*` tables in the same database file — six at Cycle 8, `registry_milkings` at step 4, and four more with milk sales ([REGISTRY_SALES.md](REGISTRY_SALES.md), migration 5), for eleven — and with them **the repo's first migration runner** — `PRAGMA user_version`, applied at `db.ts` load and callable against any handle, global rather than registry-specific. Three boundary facts matter at this level; everything else about the registry, including its schema, belongs to REGISTRY.md and is deliberately not repeated here:
...
- **The agent reads them, and only reads them.** Cycle 9 added three read tools over the animal registry and the sales cycle added four more over the destinations and the ledger; every registry write still goes through the entry screens. The diagram below is the *demo* schema, which is why the `registry_*` tables are not in it — their schema belongs to REGISTRY.md and REGISTRY_SALES.md.
- **The sales tables are the first with no `animal_id`.** `registry_destinations`, `registry_destination_prices`, `registry_dispatches` and `registry_payments` hang off a counterparty rather than an animal, which is why that work carries no step number in the registry's sequence.
