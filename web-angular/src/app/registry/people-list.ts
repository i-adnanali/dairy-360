              ...
              @for (p of list; track p.person_id) {
                ...
                >
                  ...
                  <!-- THE ZERO BRANCH IS THE ONE THAT MOVED, and it moved
                       ...
                       "text-content-secondary" and silently deleted the in-advance
                       highlight -- which is a liability marker, not a certainty
                       state, and not this phase's to remove. -->
                  <td
                    appCell
                    numeric
                    [class.text-content-secondary]="p.balance_minor < 0"
          ...
          >
            ...
            <label class="block space-y-1">
              ...
              <input
                name="role"
                [value]="engageRole()"
                (input)="engageRole.set($any($event.target).value)"
                appInput
                density="comfortable"
                class="w-full"
                placeholder="milker"
            ...
            @if (engageState.formError(['started_on', 'kind', 'person_id']); as msg) {
              <p appErrorPanel data-role="engage-form-error">{{ msg }}</p>
            }
