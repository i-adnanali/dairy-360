...
//
//   node scripts/harness-seed.mjs            # default http://localhost:6400
//   node scripts/harness-seed.mjs --wait=90  # poll until the harness is up
//   HARNESS_URL=http://localhost:6410 node scripts/harness-seed.mjs
//
...
const BASE = (process.env.HARNESS_URL ?? 'http://localhost:6400').replace(/\/$/, '');
  ...
  if (info === undefined) {
    die(
      `nothing usable answered at ${API}/storage (${lastError})` +
        ...
        `  npm run registry:harness -w server -- --port=6400 --empty`,
    );
  }
  if (info.memory !== true) {
    die(
      `REFUSING TO SEED.\n\n` +
        ...
        `  lsof -tiTCP:6400 -sTCP:LISTEN | xargs kill\n` +
        `  npm run registry:harness -w server -- --port=6400 --empty`,
    );
  }
